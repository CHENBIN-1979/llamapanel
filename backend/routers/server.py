...
SERVER_PARAMS_META = {
    ...
    "temp": {
        ...
        "description": "🎲 控制回答的\"创造力\"大小。温度越高（如1.2~1.5），AI越爱\"天马行空\"，可能用你意想不到的词；温度越低（如0.1~0.3），AI越\"循规蹈矩\"，每次回答都差不多。💡 参考值：写诗/创意写作→0.9~1.2，写代码/事实问答→0.1~0.3，通用聊天→0.7~0.8，调用工具→0.1~0.3",
        "default": "0.8",
        "placeholder": "0.8",
        "min": 0.0,
        "max": 2.0,
    ...
    "top_k": {
        ...
        "type": "number",
        "label": "Top-K",
        "description": "🎯 每次生成时，只让AI从\"候选名单\"前K个词里挑选。比如Top-K=40，AI就只能从排名前40的词语中选择，冷门词直接被排除。K越小回答越保守，K越大越有惊喜。💡 参考值：40~60（通用），10~20（严谨回答），0（禁用，让AI自由选择所有词），调用工具→10~20",
        "default": "40",
        "placeholder": "40",
        "min": 0,
        "max": 200,
    ...
    "top_p": {
        ...
        "type": "number",
        "label": "Top-P (核采样)",
        "description": "🎯 动态\"淘汰制\"——AI不断把最可能的词加起来，直到总概率达到Top-P值就停止。比如Top-P=0.9，只保留那些概率加起来占90%的词，剩下的10%冷门词全部淘汰。P越小候选词越少，回答越稳定。💡 参考值：0.9~0.95（通用），0.5~0.7（严谨回答），1.0（不限制，所有词都有机会），调用工具→0.1~0.3",
        "default": "0.95",
        "placeholder": "0.95",
        "min": 0.0,
        "max": 1.0,
    ...
    "min_p": {
        ...
        "type": "number",
        "label": "Min-P",
        "description": "🪝 强行\"踢掉\"冷门词。如果一个词的热门程度不到最热门词的min_p倍，就直接淘汰。比如min_p=0.05，最热门词概率30%，那概率低于1.5%的词全部出局。能有效防止AI突然\"抽风\"说出不着边际的话。💡 参考值：0.05~0.1（推荐，平衡安全与多样性），0.2（严格），0（不限制），调用工具→0.2~0.3",
        "default": "0.05",
        "placeholder": "0.05",
        "min": 0.0,
        "max": 1.0,
    ...
    "repeat_penalty": {
        ...
        "type": "number",
        "label": "重复惩罚",
        "description": "🔁 防止AI变\"复读机\"的力度。值越大，AI越不敢重复刚说过的话。1.0=随便重复，1.05=轻微提醒，1.1=适度避免，1.2+=坚决不重复（但可能导致话题跳跃）。💡 参考值：1.0~1.05（自由聊天），1.1~1.15（通用/写作，推荐），1.15~1.2（需要丰富词汇时），调用工具→1.0~1.05",
        "default": "1.1",
        "placeholder": "1.1",
        "min": 1.0,
        "max": 2.0,
    ...
    "repeat_last_n": {
        ...
        "type": "number",
        "label": "重复惩罚窗口",
        "description": "📏 在最近多少个字的范围内检查重复。比如设为64，AI在最近64个字内重复说某词就会被扣分。设太小查重范围窄，AI可能前后翻来覆去说同一套话；设太大会\"记性太好\"，AI可能因避免旧词而跑偏。💡 参考值：64~128（通用聊天），256+（长文创作），0（完全不查重），-1（全程检查），调用工具→64~128",
        "default": "64",
        "placeholder": "64",
        "min": -1,
        "max": 99999,
    ...
    "presence_penalty": {
        ...
        "description": "💡 鼓励AI\"聊点新东西\"。只要一个概念出现过就会被扣分，AI就会转向没用过的新词。值越大，AI越爱引入新话题、新词汇，适合头脑风暴或创意写作。但太大可能导致回答散乱。💡 参考值：0.0（不干预，默认），0.1~0.3（适当鼓励话题多样性），0.4~0.6（强烈鼓励创新，创意模式），调用工具→0.0~0.1",
        "default": "0.0",
        "placeholder": "0.0",
        "min": -2.0,
        "max": 2.0,
    ...
    "timeout": {
        ...
        "type": "number",
        "label": "模型自动卸载空闲超时 (秒)",
        "description": "当模型指定GPU层数(ngl>0)时，模型在不活动后自动从GPU卸载的空闲超时时间（秒），0=不卸载。",
        "default": "600",
        "placeholder": "600",
        "min": 10,
        "max": 86400,
