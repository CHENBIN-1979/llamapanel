...
class ModelManager:
    ...
    def update_progress(self, filename: str, percent: int, status: str, downloading: bool = True, can_resume: bool = False):
    ...
    def get_progress(self, filename: str) -> Dict:
        with self.progress_lock:
            ...
            return {'percent': 0, 'status': '未开始', 'downloading': False, 'can_resume': False}
    ...
    def download_model(self, download_url: str, filename: str, model_id: str, callback=None) -> bool:
        ...
        try:
            ...
            self.update_progress(filename, 0, "开始下载...", True, resume_byte > 0)
            if callback:
                callback(0, "开始下载...")
                ...
                with open(download_path, 'ab') as f:
                    while True:
                        ...
                        if self.download_stop_flags.get(filename, False):
                            ...
                            self.update_progress(filename, last_percent, "已暂停", False, True)
                            if callback:
                        ...
                        if total_size > 0:
                            ...
                            if percent > last_percent:
                                ...
                                self.update_progress(filename, percent, f"下载中... {percent}%", True, True)
                                if callback:
    ...
    def resume_download(self, filename: str, model_id: str) -> bool:
        """恢复暂停的下载"""
        # 重新获取下载信息
        download_url = None
        for cached_model_id in self.download_progress:
            if cached_model_id == filename:
                # 需要从缓存中获取 URL，这里简化处理
                pass
        
        # 如果没有找到，需要前端传递 download_url
        self.log(f"尝试恢复下载: {filename}")
        # 清除停止标志
        if filename in self.download_stop_flags:
            self.download_stop_flags[filename] = False
        return True
